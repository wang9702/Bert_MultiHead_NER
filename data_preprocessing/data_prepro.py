...
def encoder(sentence, argument):
    ...
    zero = [0 for _ in range(args.max_length)]
    span_mask = [attention_mask for _ in range(sum(attention_mask))]
    span_mask.extend([zero for _ in range(sum(attention_mask), args.max_length)])

    span_label = [0 for _ in range(args.max_length)]
    span_label = [span_label for _ in range(args.max_length)]
...
def data_pre(file_path, data_sign):
    data_cache = './data_preprocessing/{}.cache'.format(data_sign)
    if not os.path.exists(data_cache):
        sentences, arguments = load_data(file_path)
        data = []
        for i in tqdm(range(len(sentences))):
            encode_sent, token_type_ids, attention_mask, span_label, span_mask = encoder(sentences[i], arguments[i])
            tmp = {}
...
def yield_data(file_path, data_sign):
    tmp = MyDataset(data_pre(file_path, data_sign))
    return DataLoader(tmp, batch_size=args.batch_size, shuffle=True)


if __name__ == '__main__':
    data_sign = 'train'
    data = data_pre(args.train_path, data_sign)
