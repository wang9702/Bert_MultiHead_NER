...
class WarmUp_LinearDecay:
    def __init__(self, optimizer: optim.Adam, init_rate, warm_up_epoch, decay_epoch, min_lr_rate=1e-8):
...
def train():
    train_data = data_prepro.yield_data(args.train_path, 'train')
    test_data = data_prepro.yield_data(args.test_path, 'test')
    ...
    for epoch in range(args.epoch):
        for item in train_data:
            ...
            span_loss_v = span_loss_func(span_logits, span_label.to(device), span_mask.float().to(device))
            loss = span_loss_v
            ...
            if step % 100 == 0:
                ...
                logger.info('epoch %d, step %d, loss %.4f, recall %.4f, precise %.4f, span_f1 %.4f' % (
